...
class YStream:
    ...
    * The remaining arguments are optional knobs for controlling downloading behavior and default
    ...
    def _download_file(self, from_basename: str, to_basename: Optional[str] = None) -> str:
        ...
        errors = []
        for i in range(1 + self.download_retry):
            try:
                print(f"ygong: downloading from {remote} to {local}: retry-try: {i}")
                ...
                print(f"ygong: downloading error: {e}")
                errors.append(e)
                continue
            break
    ...
    def get_shards(self, world: World) -> List[Reader]:
        ...
        if world.is_local_leader:
            if self.remote:
                ...
                print(f"ygong: finished downloading and rename to {tmp_filename}")
            ...
            ygong_wait_for_file_to_exist(
                filename, 10, self.download_timeout,
                f'Index file {filename} took too long to download. Either ' +
                f'increase the `download_timeout` value or check the other ' + f'traceback.', world)
